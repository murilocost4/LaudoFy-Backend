  ...
  try {
    ...
    const laudosComValor = await Promise.all(
      laudos.map(async (laudo) => {
        ...
        // Tentar buscar valor configurado
        if (laudo.medicoResponsavelId && laudo.exame?.tipoExame?._id) {
        ...
        return {
          ...laudo,
          valorConfigurado: valorConfigurado || laudo.valorPago || 0,
          pacienteNome: laudo.exame?.paciente?.nome || 'N/A',
          tipoExameNome: laudo.exame?.tipoExame?.nome || 'N/A',
          medicoNome: laudo.medicoResponsavelId?.nome || 'N/A',
    ...
    console.log('Laudos retornados:', laudosComValor.length);
    console.log('Exemplo de laudo:', laudosComValor[0] ? {
      id: laudosComValor[0]._id,
      medicoId: laudosComValor[0].medicoId,
      medicoNome: laudosComValor[0].medicoNome,
      valorConfigurado: laudosComValor[0].valorConfigurado
    } : 'Nenhum laudo');
...
exports.registrarPagamento = async (req, res) => {
  ...
  try {
    ...
    if (laudosJaPagos.length > 0) {
      ...
      console.log('Laudos já pagos encontrados:', laudosJaPagosIds);
    ...
    console.log(`Processando ${laudos.length} laudos para pagamento do médico ${medicoId}`);
    ...
    const pagamento = new PagamentoLaudo({
      ...
      valorTotal: laudos.reduce((sum, laudo) => sum + (laudo.valorPago || 0), 0),
      valorDesconto: valorDesconto,
    ...
    console.log('Pagamento salvo:', pagamento._id);

    // Atualizar cada laudo individualmente usando o método específico
    for (const laudo of laudos) {
      try {
        ...
        console.log(`Laudo ${laudo._id} marcado como pago`);
      } catch (error) {
        console.error(`Erro ao registrar pagamento no laudo ${laudo._id}:`, error);
        throw error;
      }
    }
    ...
    console.log(`Verificação: ${laudosPagosContagem}/${laudos.length} laudos foram marcados como pagos`);
    ...
    try {
      ...
      const laudosInfo = laudos.map(laudo => {
        ...
        if (typeof cpfPaciente === 'string' && cpfPaciente.includes(':')) {
          try {
            ...
            console.error('Erro ao descriptografar CPF do paciente:', error);
          }
        }
      ...
      if (typeof nomeMedico === 'string' && nomeMedico.includes(':')) {
        try {
          ...
          console.error('Erro ao descriptografar nome do médico:', error);
        }
      }
      ...
      await AuditLog.create({
        ...
        action: 'create',
        description: `Pagamento registrado para ${laudos.length} laudo(s) do médico ${nomeMedico} - Valor: R$ ${valorFinal.toFixed(2)}`,
...
exports.gerarRecibo = async (req, res) => {
  try {
    ...
    doc.text(`Médico: ${pagamento.medicoId.nome}`);
    doc.text(`CRM: ${pagamento.medicoId.crm?.numero || 'N/A'}`);
    doc.moveDown();
    ...
    for (const laudoId of pagamento.laudos) {
      ...
      if (laudo) {
        const pacienteNome = laudo.exame?.paciente?.nome || 'N/A';
        const exameNome = laudo.exame?.tipoExame?.nome || 'N/A';
        const valor = laudo.valorPago || 0;
...
exports.relatorioPorMedico = async (req, res) => {
  try {
    ...
    console.error('Erro ao gerar relatório por médico:', error);
...
exports.relatorioPorTipoExame = async (req, res) => {
  try {
    ...
    console.error('Erro ao gerar relatório por tipo de exame:', error);
...
exports.listarPagamentos = async (req, res) => {
  try {
    const { page = 1, limit = 20, tenant, medico, dataInicio, dataFim, meioPagamento, medicoId } = req.query;
    ...
    if (req.usuario.role === 'adminMaster') {
      ...
      // Admin/recepcionista can only see their own tenant
    ...
    const pagamentosFormatados = pagamentos.map(pagamento => ({
      _id: pagamento._id,
      dataPagamento: pagamento.dataPagamento,
      medicoId: pagamento.medicoId?._id,
      medicoNome: pagamento.medicoId?.nome || 'N/A',
      valorTotal: pagamento.valorTotal || 0,
      valorDesconto: pagamento.valorDesconto || 0,
      valorFinal: pagamento.valorFinal || 0,
      meioPagamento: pagamento.meioPagamento || 'N/A',
      observacoes: pagamento.observacoes,
      registradorNome: pagamento.registradoPor?.nome || 'N/A',
      laudos: pagamento.laudos || [],
      laudosDetalhes: pagamento.laudos?.map(laudo => ({
        _id: laudo._id,
        pacienteNome: laudo.exame?.paciente?.nome || 'N/A',
        tipoExameNome: laudo.exame?.tipoExame?.nome || 'N/A',
    ...
    // Calculate summary statistics for the current query
    const resumoAggregate = await PagamentoLaudo.aggregate([
      { $match: query },
      {
        $group: {
          _id: null,
          ...
          valorDescontos: { $sum: '$valorDesconto' },
          valorLiquido: { $sum: '$valorFinal' }
        }
      }
    ]);
    
    const resumo = resumoAggregate[0] || {
      totalPagamentos: 0,
      valorTotal: 0,
      valorDescontos: 0,
      valorLiquido: 0
    ...
    console.log('Total de pagamentos encontrados:', pagamentosFormatados.length);
