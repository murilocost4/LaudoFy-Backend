...
exports.criarPaciente = async (req, res) => {
    try {
        ...
        const cpfExistente = await Paciente.findOne({ cpf: cpfCriptografado, tenant_id: tenantId });
        if (cpfExistente) {
...
exports.listarPacientes = async (req, res) => {
    try {
        ...
        const filtro = req.usuario?.role === 'adminMaster' ? {} : { tenant_id: req.tenant_id };
        ...
        if (cpf && cpf.trim() !== '') {
            ...
            console.log('Filtro CPF será aplicado:', termoBuscaCPF);
        }
        ...
        console.log(`Pacientes encontrados antes dos filtros: ${pacientesEncontrados.length}`);
        ...
        if (deveAplicarFiltroNome) {
            console.log('Aplicando filtro de nome:', termoBuscaNome);
            ...
            pacientesFiltrados = pacientesFiltrados.filter(paciente => {
                try {
                    ...
                    if (match) {
                        console.log(`Nome match: "${nomeDescriptografado}" contém "${termoBuscaNome}"`);
                    }
                    return match;
                } catch (error) {
            ...
            console.log(`Pacientes após filtro nome: ${pacientesFiltrados.length}`);
        }

        // Filtro por CPF (após descriptografia)
        if (deveAplicarFiltroCPF) {
            console.log('Aplicando filtro de CPF:', termoBuscaCPF);
            ...
            pacientesFiltrados = pacientesFiltrados.filter(paciente => {
                try {
                    ...
                    if (match) {
                        console.log(`CPF match: "${cpfLimpo}" contém "${termoBuscaCPF}"`);
                    }
                    return match;
                } catch (error) {
            ...
            console.log(`Pacientes após filtro CPF: ${pacientesFiltrados.length}`);
        }

        // Aplicar limite após filtros
        const pacientesPaginados = pacientesFiltrados.slice(0, limit);

        console.log(`Total final: ${pacientesPaginados.length}`);
        ...
        const pacientesFinais = pacientesPaginados.map(paciente => {
            ...
            if (pacienteFormatado.nome && typeof pacienteFormatado.nome === 'string' && pacienteFormatado.nome.includes(':')) {
                try {
                    ...
                    console.error('Error decrypting nome:', err);
        ...
        console.log('Resultado final:', {
            pacientes: pacientesFinais.length,
            total: pacientesFiltrados.length
        });

        // **Debug: Log do primeiro paciente**
        if (pacientesFinais.length > 0) {
            console.log('Primeiro paciente resultado:', {
                id: pacientesFinais[0]._id,
                nome: pacientesFinais[0].nome,
                cpf: pacientesFinais[0].cpf ? pacientesFinais[0].cpf.substring(0, 3) + '***' : 'N/A',
                tenant: pacientesFinais[0].tenant_id?.nomeFantasia
            });
        }
