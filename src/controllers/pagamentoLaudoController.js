...
exports.listarLaudosPorMedico = async (req, res) => {
  try {
    ...
    const laudosFormatados = laudos.map(laudo => {
      const laudoFormatado = { ...laudo };
      
      // Descriptografar nome do médico
      if (laudoFormatado.medico?.nome) {
        try {
          ...
          console.error('Erro ao descriptografar nome do médico:', err);
    ...
    console.error('Erro ao listar laudos por médico:', error);
    res.status(500).json({ 
...
exports.listarPagamentos = async (req, res) => {
  try {
    ...
    const pagamentosFormatados = pagamentos.map(pagamento => {
      const pagamentoFormatado = { ...pagamento };
      ...
      // Descriptografar nome do usuário que registrou
      if (pagamentoFormatado.registradoPor?.nome) {
        try {
          ...
          console.error('Erro ao descriptografar nome do usuário:', err);
